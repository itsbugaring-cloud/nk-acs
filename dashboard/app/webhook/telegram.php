...
function showDeviceLocation($deviceId, $chatId, $messageId, $telegram, $conn) {
    $message = "ℹ️ <b>Lokasi Perangkat</b>\n\n";
    $message .= "Fitur topologi/map saat ini dinonaktifkan dari NETKING-ACS.\n";
    $message .= "Gunakan menu perangkat, OLT inventory, dan alarm untuk operasional harian.";
    $telegram->editMessage($messageId, $chatId, $message, $telegram->getDeviceDetailKeyboard($deviceId));
}

function sendDeviceGPSLocation($deviceId, $chatId, $telegram, $conn) {
    $telegram->sendMessage("ℹ️ Fitur kirim GPS dinonaktifkan karena modul topologi/map sudah dipensiunkan.", $chatId);
}

...
function showItemListModal(itemType) {
    ...
    // Prefer real OLT map items. Fall back to legacy server.properties.olt_link
    // only when no dedicated OLT items exist yet.
    if (itemType === 'olt') {
    ...
    if (items.length === 0) {
        ...
        items.forEach(item => {
            ...
            if (itemType === 'olt') {
                ...
                const oltLink = item.config?.olt_link || item.properties?.olt_link || 'N/A';
                const ponPorts = item.config?.pon_ports || {};
                const ponPortCount = item.config?.pon_count || Object.keys(ponPorts).length || 0;

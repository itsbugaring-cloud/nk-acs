...
async function loadMap() {
    ...
    if (itemsResult && itemsResult.success) {
        ...
        items.forEach(item => {
            ...
            // Skip hidden items (registry-only entities, generated helpers, etc.)
            const properties = item.properties || {};
            if (properties.hidden_marker) {
                return; // Keep item in DB, but don't render marker in map
            }
...
function updateItemCounters() {
    ...
    // Legacy fallback: older deployments represented OLTs on server properties.
    if (counts.olt === 0) {
        allMapItems.forEach(item => {
            if (item.item_type === 'server' && item.properties && item.properties.olt_link && item.properties.olt_link.trim() !== '') {
                counts.olt++;
            }
        });
    }

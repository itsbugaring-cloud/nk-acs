...
<?php else: ?>
    <?php if (!isCpeWriteEnabled()): ?>
        <div class="alert alert-info mb-3">
            <i class="bi bi-shield-lock"></i>
            Safe Mode aktif: aksi write ke ONT (reboot/WAN/WiFi/DHCP/factory reset) sedang dikunci untuk keamanan pelanggan.
        </div>
    <?php endif; ?>
    <div class="card devices-screen">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <i class="bi bi-router"></i> Device List
                    <span id="device-stats-badges" class="device-stats-badges" style="margin-left: 10px;">
                ...
                <div>
                    ...
                    <div id="bulk-action-buttons" class="bulk-action-buttons" style="display: none; margin-right: 10px;">
        ...
        <div class="card-body">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    <div class="d-flex flex-wrap align-items-center gap-2" id="sticky-filter-bar">
                        <span class="text-muted small">Quick Filter:</span>
                        <button class="btn btn-sm btn-outline-secondary" onclick="applyQuickFilter('all')">Semua</button>
                        <button class="btn btn-sm btn-outline-success" onclick="applyQuickFilter('online')">Online</button>
                        <button class="btn btn-sm btn-outline-danger" onclick="applyQuickFilter('offline')">Offline</button>
                        <button class="btn btn-sm btn-outline-danger" onclick="applyQuickFilter('rx_critical')">RX Critical</button>
                        <button class="btn btn-sm btn-outline-warning" onclick="applyQuickFilter('rx_warning')">RX Warning</button>
                        <button class="btn btn-sm btn-outline-primary" onclick="applyQuickFilter('high_clients')">Client Tinggi</button>
                        <button class="btn btn-sm btn-dark" onclick="saveCurrentDeviceView()">Simpan View</button>
                        <div id="saved-device-views" class="d-flex flex-wrap gap-2 ms-2"></div>
                    </div>
                </div>
            </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="deviceInspectorDrawer" aria-labelledby="deviceInspectorTitle">
        <div class="offcanvas-header">
            <div>
                <h5 class="offcanvas-title" id="deviceInspectorTitle">Device Inspector</h5>
                <div class="text-muted small">Ringkasan read-only ACS/OLT.</div>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body" id="device-inspector-content">
            <div class="text-muted">Pilih ONT untuk melihat detail cepat.</div>
        </div>
    </div>
...
<script src="/assets/js/devices/devices-state.js?v=<?php echo time(); ?>"></script>

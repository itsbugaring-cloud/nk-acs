...
function calculateOLTStatus($item, $allItems, $netwatchStatus) {
    ...
    $syncState = $properties['inventory_sync_state'] ?? null;
    $lastAttemptAt = $properties['inventory_last_attempt_at'] ?? null;
    if ($syncState && $lastAttemptAt) {
        $lastAttemptTs = strtotime((string) $lastAttemptAt);
        if ($lastAttemptTs !== false) {
            $ageSeconds = time() - $lastAttemptTs;
            if ($ageSeconds <= 2700) {
